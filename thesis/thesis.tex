...
\newcommand{\profColab}{Walter Blanco Vino}
...
El \textbf{coeficiente de correlación de Pearson} (\(r\)) cuantifica la intensidad y dirección de la relación lineal entre dos variables cuantitativas. Sus valores se encuentran acotados entre \(-1\) y \(1\), donde cifras próximas a \(1\) reflejan una correlación lineal positiva fuerte, valores cercanos a \(-1\) indican una correlación lineal negativa fuerte y valores próximos a \(0\) sugieren ausencia de relación lineal. La formulación utilizada se presenta en la \autoref{eq:pearson_coef}, donde \(x_i\) y \(y_i\) corresponden a las observaciones de cada variable y \(\bar{x}\) y \(\bar{y}\) representan sus respectivas medias \parencite{montgomery2018}.

\begin{equation}
r =
\frac{\sum_{i=1}^{n}(x_i - \bar{x})(y_i - \bar{y})}
{\sqrt{\sum_{i=1}^{n}(x_i - \bar{x})^2 \sum_{i=1}^{n}(y_i - \bar{y})^2}}
\label{eq:pearson_coef}
\end{equation}
...
De forma complementaria, se incorporó el \textbf{coeficiente de correlación de Spearman} (\( \rho_s \)), una medida no paramétrica que evalúa la relación monótona entre dos variables a partir del orden de sus valores, sin requerir supuestos de normalidad ni homocedasticidad \parencite{field2018}. Este coeficiente se define como la correlación de Pearson aplicada sobre los rangos de las observaciones, donde \( d_i \) representa la diferencia entre los rangos de cada par de observaciones y \( n \) el número total de pares analizados. Al basarse en rangos, esta medida es más robusta frente a valores atípicos y distribuciones no normales, siendo especialmente útil cuando la relación entre las variables es creciente o decreciente, pero no estrictamente lineal (\autoref{eq:spearman_coef}).

\begin{equation}
\rho_s = 1 - \frac{6 \sum_{i=1}^{n} d_i^2}{n(n^2 - 1)}
\label{eq:spearman_coef}
\end{equation}
...
Además, se construyó una \textbf{matriz de confusión}, la cual permite identificar el número de aciertos y errores cometidos en cada clase —“ocurre” y “no ocurre”—, proporcionando una evaluación más completa del desempeño del modelo. Este análisis resulta fundamental para reconocer sus limitaciones y capacidades predictivas, especialmente en contextos aplicados a la detección de eventos de tránsito.

Las métricas empleadas para cuantificar el desempeño del modelo se definen en la \autoref{eq:metrics_confusion}, donde \(TP\), \(TN\), \(FP\) y \(FN\) corresponden a los verdaderos positivos, verdaderos negativos, falsos positivos y falsos negativos, respectivamente.
...
\begin{figure}[H]
    ...
    \caption{Distribución espacial de eventos (a: Accidentes, b: Congestión, c: Peligros)}
    \label{fig:dist_events}
\end{figure}

Para el análisis temporal, los eventos se agruparon por hora del día, calculando el promedio de ocurrencias por jornada. En la \autoref{fig:time_events} se presenta la distribución horaria completa. La \autoref{fig:time_events_a} evidencia que los accidentes presentan una concentración marcada durante las horas punta; la \autoref{fig:time_events_b} muestra un comportamiento similar en el caso de las congestiones, mientras que la \autoref{fig:time_events_c} revela que los eventos de tipo \texttt{peligro} mantienen una distribución más uniforme a lo largo del día.

\begin{figure}[H]
    \centering
    \subfloat[Accidentes\label{fig:time_events_a}]{
        \includegraphics[width=0.6\textwidth]{images/ACCIDENT_per_hour.png}
    }
    \newline
    \subfloat[Congestión\label{fig:time_events_b}]{
        \includegraphics[width=0.6\textwidth]{images/JAM_per_hour.png}
    }
    \newline
    \subfloat[Peligros\label{fig:time_events_c}]{
        \includegraphics[width=0.6\textwidth]{images/HAZARD_per_hour.png}
    }
    \newline
    \caption{Eventos según hora del día (a: Accidentes, b: Congestión, c: Peligros)}
...
La hipótesis nula (\autoref{eq:h0_corr}) establece que no existe correlación lineal entre ambas variables, mientras que la hipótesis alternativa (\autoref{eq:h1_corr}) plantea la existencia de una correlación distinta de cero. En estas expresiones, \(\rho\) corresponde al coeficiente de correlación poblacional.
...
El estadístico de prueba utilizado para evaluar la significancia del coeficiente de correlación se obtiene a partir de la expresión mostrada en la \autoref{eq:stat_t}. En esta formulación, el valor de $t$ depende del coeficiente de correlación muestral $r$ y del tamaño de la muestra $n$:
...
Con el propósito de caracterizar y cuantificar la relación observada entre las variables, se ajustó un modelo de regresión lineal simple cuya formulación se presenta en la \autoref{eq:reg_lineal}. En este modelo, la variable respuesta \(y\) corresponde al número promedio de accidentes por hora, mientras que \(x\) representa el número promedio de reportes de congestión por hora.
...
Los parámetros \(\beta_0\) y \(\beta_1\) describen la estructura sistemática de la relación: el primero actúa como intercepto, reflejando el valor esperado de \(y\) cuando \(x = 0\); el segundo corresponde a la pendiente o coeficiente de regresión, indicando el cambio promedio en \(y\) ante una variación unitaria en \(x\). El término \(\varepsilon\) captura la componente aleatoria del modelo, la cual se asume con esperanza cero y varianza constante.
...
\begin{figure}[H]
    ...
    \newline
    \caption{Modelos de regresión lineal entre accidentes y reportes de congestión (a: Día de semana, b: Fin de semana)}
    \label{fig:corr_lineal}
\end{figure}

El análisis estadístico del modelo lineal arrojó estimaciones diferenciadas para días de semana y fines de semana. Para los \textbf{días de semana}, el modelo ajustado se presenta en la \autoref{eq:lin_weekdays}, mientras que los indicadores asociados muestran un coeficiente de determinación elevado (\(R^2 = 0.92\)), una significancia global del modelo altamente robusta (\(F(1,22) = 256.70,\ p < 0.0001\)) y parámetros estadísticamente significativos tanto para el intercepto (\(\beta_0\), \(t = 4.08,\ p < 0.001\)) como para la pendiente (\(\beta_1\), \(t = 16.02,\ p < 0.0001\)).

\begin{equation}
y = 0.1382\,x + 0.1072
\label{eq:lin_weekdays}
\end{equation}

En el caso de los \textbf{fines de semana}, el modelo estimado se muestra en la \autoref{eq:lin_weekend}. Los resultados indican un ajuste más moderado que en días laborales, con un coeficiente de determinación de \(R^2 = 0.75\), aunque igualmente acompañado de una alta significancia global (\(F(1,22) = 66.23,\ p < 0.0001\)). Tanto el intercepto como la pendiente también resultaron estadísticamente significativos (\(\beta_0\), \(t = 9.24,\ p < 0.0001\); \(\beta_1\), \(t = 8.14,\ p < 0.0001\)).

\begin{equation}
y = 0.1205\,x + 0.0719
\label{eq:lin_weekend}
\end{equation}

En conjunto, los valores elevados de \(R^2\) y la significancia de los parámetros confirman que el número de reportes de congestión explica una fracción considerable de la variabilidad observada en la frecuencia de accidentes por hora. No obstante, es importante enfatizar que la existencia de una relación lineal no implica causalidad; la interpretación final requiere contrastar estos resultados con información contextual adicional y con otros factores potencialmente relevantes.
...
El análisis de los residuos del modelo lineal reveló indicios de una estructura no capturada completamente por una relación estrictamente lineal, especialmente durante los fines de semana. Esta evidencia motivó la exploración de un modelo no lineal de tipo exponencial inverso, apropiado para representar situaciones en las que el efecto marginal de la congestión sobre la ocurrencia de accidentes disminuye progresivamente a medida que aumenta la densidad vehicular.

La forma funcional considerada se presenta en la \autoref{eq:exp_inv}, donde \(A(x)\) denota el número estimado de accidentes por hora para un nivel dado de congestión.

\begin{equation}
A(x) = \frac{\alpha}{x^{\beta}} + \gamma + \varepsilon
\label{eq:exp_inv}
\end{equation}

En este modelo, \(x\) corresponde al número de reportes de congestión por hora. Los parámetros \(\alpha\), \(\beta\) y \(\gamma\) se estiman mediante regresión no lineal: \(\alpha\) actúa como parámetro de escala que controla la magnitud inicial del efecto; \(\beta\) determina la tasa de decrecimiento del impacto de la congestión sobre la ocurrencia de accidentes; y \(\gamma\) introduce un desplazamiento vertical que permite capturar un nivel base de accidentes independiente del nivel de congestión observado. Finalmente, \(\varepsilon\) representa el término de error aleatorio del modelo.
...
\begin{figure}[H]
    ...
    \newline
    \caption{Correlación exponencial inversa entre accidentes y congestión (a: Día de semana, b: Fin de semana)}
    \label{fig:corr_exp}
\end{figure}

El análisis estadístico del modelo exponencial inverso mostró un ajuste significativo tanto para días de semana como para fines de semana. Para los \textbf{días de semana}, el modelo estimado corresponde a la forma presentada en la \autoref{eq:invexp_weekdays}. Los resultados indican un coeficiente de determinación elevado (\(R^2 = 0.9520,\ R^2_{\text{aj}} = 0.9499\)), junto con una significancia global robusta del modelo (\(F(2,22) = 218.32,\ p < 0.0001\)). Los parámetros obtenidos fueron estadísticamente significativos, con \(a = 0.2821\) (\(t = 13.80,\ p < 0.0001,\ \text{IC}_{95\%} = [0.2397,\;0.3245]\)) y \(b = -0.6468\) (\(t = -13.50,\ p < 0.0001,\ \text{IC}_{95\%} = [-0.7461,\;-0.5475]\)).

\begin{equation}
y = 0.2821\,x^{-0.6468}
\label{eq:invexp_weekdays}
\end{equation}

En el caso de los \textbf{fines de semana}, el modelo ajustado —mostrado en la \autoref{eq:invexp_weekend}— refleja un comportamiento similar pero menos pronunciado. El ajuste es moderadamente alto (\(R^2 = 0.7838,\ R^2_{\text{aj}} = 0.7740\)) y también presenta una significancia global elevada (\(F(2,22) = 39.89,\ p < 0.0001\)). Los parámetros estimados resultaron significativos, con \(a = 0.1781\) (\(t = 20.55,\ p < 0.0001,\ \text{IC}_{95\%} = [0.1602,\;0.1961]\)) y \(b = -0.3008\) (\(t = -7.48,\ p < 0.0001,\ \text{IC}_{95\%} = [-0.3841,\;-0.2174]\)).

\begin{equation}
y = 0.1781\,x^{-0.3008}
\label{eq:invexp_weekend}
\end{equation}

En conjunto, estos resultados evidencian diferencias claras en la sensibilidad de la relación congestión–accidentes entre ambos periodos. Durante los días de semana, el exponente de mayor magnitud en valor absoluto refleja una caída más marcada del efecto marginal conforme aumenta la congestión, mientras que en fines de semana la relación es sustancialmente más suave.
...
En este contexto, se aplicó el \textbf{coeficiente de correlación de Spearman} (\(\rho_s\)), una medida no paramétrica que cuantifica la fuerza y dirección de la relación monótona entre dos variables a partir del orden relativo de sus observaciones. Este enfoque resulta especialmente pertinente cuando la relación puede ser creciente o decreciente, pero no estrictamente lineal, tal como ocurre en fenómenos de tráfico urbano donde el aumento de la congestión no necesariamente implica una variación proporcional en la ocurrencia de accidentes.

La definición formal de \(\rho_s\) corresponde a la correlación de Pearson calculada sobre los rangos de las observaciones, cuya formulación se presenta en la \autoref{eq:spearman}.
...
En esta expresión, \(d_i\) denota la diferencia entre los rangos asignados a las observaciones \(x_i\) y \(y_i\), mientras que \(n\) corresponde al número total de pares observados. El valor de \(\rho_s\) se encuentra acotado entre \(-1\) y \(1\), representando respectivamente una relación monótona negativa perfecta, ausencia de correlación monótona, o una relación monótona positiva perfecta.
...
\begin{figure}[H]
    ...
    \caption{Intersecciones críticas — detalle con \textit{buffer} alrededor del centro del segmento (\textbf{a}: Nicolás Tirado / Pedro Aguirre Cerda, \textbf{b}: Salvador Allende / Edmundo Pérez Zujovic, \textbf{c}: Caparrosa / Pedro Aguirre Cerda, \textbf{d}: Tamarugos / Edmundo Pérez Zujovic)}
\end{figure}
...
Los hiperparámetros óptimos obtenidos para el modelo \textit{Random Forest} se presentan en la \autoref{tab:rf-params}.
...
Los mejores hiperparámetros para la \textit{Regresión Logística} se muestran en la \autoref{tab:lr-params}.
...
El modelo obtuvo un \textbf{ROC–AUC de 0.7463} y un \textbf{PR–AUC de 0.6590} (véase \autoref{tab:model-comparison}).

\paragraph{XGBoost.}

En la \autoref{tab:xgb-params} se resumen los hiperparámetros óptimos obtenidos para el modelo \textit{XGBoost}.
...
Con esta configuración, \textit{XGBoost} alcanzó la mejor puntuación promedio, con un \textbf{ROC–AUC de 0.8433} y un \textbf{PR–AUC de 0.8108}, superando ligeramente a \textit{Random Forest} y de forma clara a la \textit{Regresión Logística} (véase \autoref{tab:model-comparison}).

La comparación global de los tres modelos se resume en la \autoref{tab:model-comparison}. Asimismo, las curvas ROC y Precision–Recall se muestran en las \autoref{fig:ROC} y \autoref{fig:prec-recall}, respectivamente.
...
Dado que el conjunto original contiene únicamente eventos positivos (ocurridos), fue necesario generar instancias negativas (no ocurridos) para entrenar un modelo de clasificación binaria. Para ello, se implementó una estrategia inspirada en el enfoque de generación de eventos negativos artificiales, el cual permite representar problemas secuenciales como tareas de clasificación supervisada \parencite{goedertier2009robust}.\footnote{Este enfoque evita el uso de sobremuestreo sintético como SMOTE, reduciendo la distorsión de la distribución original de los datos.}
...
El procedimiento se resume en el \autoref{alg:neg_events}. El código completo se incluye en la \autoref{annex:neg_code}.
...
La versión final del modelo se entrenó utilizando los hiperparámetros obtenidos durante la etapa de validación cruzada. El modelo fue registrado y versionado mediante \textit{MLflow}, permitiendo un seguimiento detallado de sus parámetros y métricas.
...
El sistema se encuentra desplegado en línea y disponible públicamente a través del sitio web \url{https://traficoantofagasta.com}. La estructura técnica y la arquitectura del sistema (cliente y \textit{backend}) se describen de forma general en la \autoref{annex:dashboard}.
...
\begin{figure}[H]
    ...
    \caption{Vista general del \textit{dashboard} desarrollado para el monitoreo y predicción del tráfico vehicular (a: Sección de análisis descriptivo, b: Sección del modelo predictivo)}
...
\subsection{Transformación de coordenadas UTM}
\label{ap:utm}
...
La expresión utilizada para obtener la coordenada proyectada \(y\) se muestra en la \autoref{eq:coord_y}. Esta corresponde a un desarrollo en serie derivado de una expansión de Taylor, lo que permite transformar con alta precisión posiciones definidas sobre la superficie curva del elipsoide hacia un sistema plano cartesiano.
...
Estas fórmulas corresponden al desarrollo clásico documentado por Snyder \parencite{snyder1987}, y constituyen la base de las transformaciones implementadas por la biblioteca \texttt{PROJ}.

\subsection*{Nota}

La implementación computacional de esta formulación puede realizarse mediante bibliotecas como \texttt{Pyproj}, las cuales encapsulan estos desarrollos y gestionan automáticamente los parámetros específicos del sistema de referencia de coordenadas (CRS) empleado.

\subsection{Código fuente: Filtrado temporal y agrupación de eventos}
...
\subsection{Código fuente: Generación de eventos negativos}
...
\subsection{Estructura general del sistema y despliegue}
